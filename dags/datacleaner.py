def data_cleaner():
  ...
  # Removes any special symbols from STORE_LOCATION column
  def clean_store_location(st_loc):
    return re.sub(r'[^\w\s]', '', st_loc).strip()

  # Removes any non-integer characters from PRODUCT_ID column
  ...
  # Remove any dollar sign from the price columns
